    ...
    try:
        ...
        for link_name in sphere_links:
            ...
            if link_start and link_end:
                ...
                print(f"  • {link_name}: {link_sphere_count} spheres (expected: {expected_spheres}), {link_collisions} total collisions, {link_mesh_collisions} mesh refs")
        print()
        
        # Overall verification
        export_valid = (
            has_xml_declaration and
            sphere_count == len(app.sphere_store.by_id) and
            collision_count == len(app.sphere_store.by_id) and
            old_mesh_collisions == 0
        )
